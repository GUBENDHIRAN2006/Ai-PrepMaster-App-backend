...
# Use Integer for primary keys in SQLite (BigInteger doesn't auto-increment in SQLite),
# and BigInteger for PostgreSQL / Supabase where larger IDs are expected.
_PK  = Integer if is_sqlite else BigInteger
_FK  = Integer if is_sqlite else BigInteger


class User(Base):
    __tablename__ = "users"

    id         = Column(_PK, primary_key=True, index=True)
...
class Profile(Base):
    ...
    id      = Column(_PK, primary_key=True, index=True)
    user_id = Column(_FK, ForeignKey("users.id"), nullable=False, unique=True)
...
class Submission(Base):
    ...
    id           = Column(_PK, primary_key=True, index=True)
    user_id      = Column(_FK, ForeignKey("users.id"),      nullable=False)
    challenge_id = Column(_FK, ForeignKey("challenges.id"), nullable=False)
